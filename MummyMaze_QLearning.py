...
# 墙壁坐标
# 阻挡了向右运动的竖直方向的墙
vwallR = [(0, 4), (1, 6), (1, 8), (3, 8), (4, 6), (5, 3), (5, 8), (7, 0), (7, 4), (7, 5), (7, 7), (8, 3), (8, 5),
          (9, 0)]
# 阻挡了向下运动的水平方向的墙
hwallD = [(0, 0), (0, 7), (0, 8), (1, 3), (2, 4), (2, 7), (3, 4), (3, 7), (4, 3), (4, 8), (5, 2), (6, 2), (7, 3),
          (7, 8), (7, 9), (8, 1), (8, 4), (7, 2)]
# 阻挡了向左运动的竖直方向的墙
vwallL = [(0, 5), (1, 7), (1, 9), (3, 9), (4, 7), (5, 4), (5, 9), (7, 1), (7, 5), (7, 6), (7, 8), (8, 4), (8, 6),
          (9, 1)]
# 阻挡了向上运动的水平方向的墙
hwallU = [(1, 0), (1, 7), (1, 8), (2, 3), (3, 4), (3, 7), (4, 4), (4, 7), (5, 3), (5, 8), (6, 2), (7, 2), (8, 3),
          (8, 8), (8, 9), (9, 1), (9, 4), (8, 2)]
...
class PLAYER_STATE:
    ...
    def nxt_position(self, action):
        # 向下、向右、向上、向左走不通
        if ((action == 'D') and (self.state in hwallD)) or ((action == 'R') and (self.state in vwallR)) or (
                (action == 'U') and (self.state in hwallU)) or ((action == 'L') and (self.state in vwallL)):
            return self.state
    ...
    # 奖励设置
    def give_reward(self, isCaught, list):
        if self.state == win:
            print("win")
            return 100
...
class Player:
    ...
    def chooseAction(self):
        ...
        if rp <= self.exp_rate / 2:
            # 随机选择下一步
            # action = np.random.choice(self.action_list)
            action = 'R'

        elif (rp > self.exp_rate and rp <= self.exp_rate):
            action = 'D'
        # 否则哪里分高走哪里
        else:
            index = np.argmax(
                self.q_table[self.State.state[0], self.State.state[1], :])
            if index == 0:
                action = 'U'
            elif index == 1:
                action = 'D'
    ...
    def play(self, mummy, sc, tries=10):
        ...
        while i < tries:
            if self.State.isEnd:
                ...
                self.exp_rate *= self.decay_factor
                ...
                hwallU = [(1, 0), (1, 7), (1, 8), (2, 3), (3, 4), (3, 7), (4, 4), (4, 7), (5, 3), (5, 8), (6, 2),
                          (7, 2), (8, 3), (8, 8), (8, 9), (9, 1), (9, 4), (8, 2)] if (onkey % 2 == 0) else [(1, 0),
                                                                                                            (1, 7),
                                                                                                            (1, 8),
                                                                                                            (2, 3),
                                                                                                            (3, 4),
                                                                                                            ...
                                                                                                            (9, 1),
                                                                                                            (9, 4)]
                hwallD = [(0, 0), (0, 7), (0, 8), (1, 3), (2, 4), (2, 7), (3, 4), (3, 7), (4, 3), (4, 8), (5, 2),
                          (6, 2), (7, 3), (7, 8), (7, 9), (8, 1), (8, 4), (7, 2)] if (onkey % 2 == 0) else [(0, 0),
                                                                                                            (0, 7),
                                                                                                            (0, 8),
                                                                                                            (1, 3),
                                                                                                            (2, 4),
                ...
                # 给予奖励
                reward = self.State.give_reward(self.isCaught, self.steps_list)
                total_reward += reward
                self.q_table[prevState[0]][prevState[1]][self.action2Index(action)] += \
                    self.lr * (reward + self.discount_factor * np.max(
                        self.q_table[self.State.state[0], self.State.state[1], :]) -
                               self.q_table[prevState[0]][prevState[1]][self.action2Index(action)])
